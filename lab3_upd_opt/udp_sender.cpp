...
static int window_size = 8;           // 初始滑动窗口大小,nowindex-base<window_size
static int ss_thresh = 128;           //窗口阈值
...
void recv_ack(int group_size)
{
    ...
    while (!tp_file_end_flag)
    {
        if (clock() - start > TIMEOUT)
        {
            ...
            if (window_queue.size() > ss_thresh + 3)
                window_size = window_queue.size(); //快速恢复
            else
                window_size = ss_thresh + 3;

            printf("已执行快速恢复,ss_thresh:%d,window:%d\n", ss_thresh, window_size);
        ...
        else // 收到正确回复，把base向前移动
        {
            ...
            printf("移动完成，当前队列大小：%d,窗口大小:%d,当前base:%d\n", window_queue.size(), window_size, window_base);

            if (window_size < ss_thresh)
                window_size *= 2; //指数增加,慢启动（其实是快启动）
            else
                window_size += 1; //线性增加,拥塞避免
            mutex_for_queue.unlock();
...
void send_file(int group_size, int size)
{
    ...
    for (now_index = 1; !tp_file_end_flag;) // index表示包的seq，理论上结束的时候now_index==group_size+1
    {
        ...
        printf("第%d个文件分组发送完成,当前队列大小%d,窗口大小%d,base:%d....\n", now_index, window_queue.size(), window_size, window_base);
        mutex_for_queue.unlock();
        now_index++; // 准备发送下一个
    }
    cout << "\n传输完成!!!\n";
