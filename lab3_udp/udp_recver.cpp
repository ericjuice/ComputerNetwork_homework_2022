...
bool rec_file()
{
    ...
    // string fpath = "D:\\MSvscodeprojects\\c_projects\\Cpt_ntw\\jw\\lab3_udp\\recvfile\\";
    ...
    for (int i = 1; i < group_size + 1; i++) // i表示希望recv包的seq
    {
        printf("第%d个文件分组接受中....\n", i);
        // client不需要 timer
        int max_err_time = 0;
        while (1)
        {
            ...
            if (recvbuff.seq != i && checksum(&recvbuff, sizeof(recvbuff)) == 0)
            {
                ...
                cout << "收到错误，重新请求对方发送\n";
                memset(&sendbuff, '\0', msg_size);
                packdata(&sendbuff, myip, desip, myport, desport, 0, 0, 0, 0, i, 0, 0, NULL);
                if (sendto(sockSend, (char *)&sendbuff, msg_size, 0, (struct sockaddr *)&address_des, addlen) == -1)
                {
                    cout << "send error";
                    return false;
            ...
            else
            {
                cout << "文件正常,回复ack\n";
                memset(&sendbuff, '\0', msg_size);
                packdata(&sendbuff, myip, desip, myport, desport, 1, 0, 0, 0, i, 0, 0, NULL);
                if (sendto(sockSend, (char *)&sendbuff, msg_size, 0, (struct sockaddr *)&address_des, addlen) == -1)
                {
                    cout << "send error";
                    return false;
